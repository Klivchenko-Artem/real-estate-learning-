CKEDITOR.dialog.add('videoDialog', function(editor) {
	...
	var lex = editor.plugins.reka.lex;
	...
	var detector = function(embedCode) {
		...
		if (embedCode.indexOf('vk.com') >= 0) {
			reproductor = 'vk';

			if (embedCode.indexOf('</iframe>') >= 0) {
				embedCode = embedCode.substring(embedCode.lastIndexOf('src="') + 5, embedCode.length).split('"')[0];
			}

			console.log(embedCode);
			if (embedCode.indexOf('video_ext') >= 0) {
				id = {
					0: embedCode.substring(embedCode.lastIndexOf('oid=') + 4, embedCode.length),
					1: embedCode.substring(embedCode.lastIndexOf('id=') + 3, embedCode.length),
				};
			} else {
				id = embedCode.substring(embedCode.lastIndexOf('video') + 5, embedCode.length).split('_');
			}
			console.log(id);
		}
	...
	var getVideoSrc = function(embedCode) {
		...
		switch (respuesta.reproductor) {
			...
			case 'dailymotion':
				...
				break;
			case 'vk':
				src = `https://vk.com/video_ext.php?oid=${respuesta.id[0]}&id=${respuesta.id[1]}`;
				break;
		}
